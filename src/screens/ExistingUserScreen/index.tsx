...
const ExistingUserScreen = () => {
  ...
  return (
    <PrimaryView colors={colors} style={gs.justifyBetween}>
      <View>
          <View style={gs.pt10p}>
            <PrimaryText size={28} weight="bold">恢复你的</PrimaryText>
            <PrimaryText size={28} weight="bold">数据</PrimaryText>
          </View>
          ...
          {fileName && !isSyncing && (
            <TouchableOpacity onPress={reUpload} style={gs.mt10}>
              <PrimaryText size={12} color={colors.accentOrange}>上传其他文件</PrimaryText>
            </TouchableOpacity>
          )}
          ...
          {showSyncProgress && (
            <View style={gs.mt30}>
              <PrimaryText size={14} weight="semibold" color={isSyncComplete ? colors.accentGreen : colors.primaryText} style={gs.mb10}>
                {isSyncComplete ? '全部完成' : 'Syncing your 数据...'}
              </PrimaryText>

              <SyncStatusItem label="个人资料" status={syncStatus.user} colors={colors} />
              <SyncStatusItem label="Categories" status={syncStatus.categories} count={syncStats.categories} colors={colors} />
              <SyncStatusItem label="支出" status={syncStatus.expenses} count={syncStats.expenses} colors={colors} />
              <SyncStatusItem label="债务人" status={syncStatus.debtors} count={syncStats.debtors} colors={colors} />
